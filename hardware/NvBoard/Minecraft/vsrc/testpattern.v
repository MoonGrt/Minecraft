...
begin
	if(!I_rst_n)
		...
		begin
			if((V_cnt >= (I_v_total-'b1)) && (H_cnt >= (I_h_total-'b1)))
				V_cnt <= 'd0;
			else if(H_cnt >= (I_h_total-'b1))
				V_cnt <=  V_cnt + 'b1;
			else
				V_cnt <= V_cnt;
		end
...
//-------------------------------------------------------------
assign  Pout_de_w = ((H_cnt>=(I_h_sync+I_h_bporch))&(H_cnt<=(I_h_sync+I_h_bporch+I_h_res-'b1)))&
                    ((V_cnt>=(I_v_sync+I_v_bporch))&(V_cnt<=(I_v_sync+I_v_bporch+I_v_res-'b1))) ;
assign  Pout_hs_w =  ~(H_cnt<=(I_h_sync-'b1));
assign  Pout_vs_w =  ~(V_cnt<=(I_v_sync-'b1));
...
begin
	if(!I_rst_n)
		Color_trig_num <= 'd0;
	else if (Pout_de_dn[1] == 'b0)
		Color_trig_num <= I_h_res[11:3]; // 8色彩条宽度
	else if ((Color_trig == 'b1) && (Pout_de_dn[1] == 'b1))
...
begin
	if(!I_rst_n)
		Color_trig <= 'b0;
	else if (De_hcnt == (Color_trig_num-'b1))
		Color_trig <= 'b1;
	else
		Color_trig <= 'b0;
end
...
begin
	if(!I_rst_n)
		Color_bar <= 'b0;
	else if(Pout_de_dn[2] == 'b1)
		case(Color_cnt)
			'd0	:	Color_bar	<=	WHITE  ;
			'd1	:	Color_bar	<=	YELLOW ;
			'd2	:	Color_bar	<=	CYAN   ;
			'd3	:	Color_bar	<=	GREEN  ;
			'd4	:	Color_bar	<=	MAGENTA;
			'd5	:	Color_bar	<=	RED    ;
			'd6	:	Color_bar	<=	BLACK  ;
			'd7	:	Color_bar	<=	BLUE  ;
...
begin
	if(!I_rst_n)
		Net_h_trig <= 'b0;
	else if (((De_hcnt[4:0] == 5'd0) || (De_hcnt == (I_h_res-'b1))) && (Pout_de_dn[1] == 'b1))
		Net_h_trig <= 'b1;
	else
		Net_h_trig <= 'b0;
end
...
begin
	if(!I_rst_n)
		Net_v_trig <= 'b0;
	else if (((De_vcnt[4:0] == 5'd0) || (De_vcnt == (I_v_res-'b1))) && (Pout_de_dn[1] == 'b1))
		Net_v_trig <= 'b1;
	else
		Net_v_trig <= 'b0;
end
