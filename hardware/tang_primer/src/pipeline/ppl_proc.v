...
/*  Break down direct division into multiplication + reciprocal
    // Step 1
    ...
    reg [31:0] recip_x_d1 = 'b0;
    reg [31:0] recip_y_d1 = 'b0;
    reg [31:0] recip_z_d1 = 'b0;
    ...
    reg [19:0] min = 'b0;
    reg [15:0] div_edge_x_d2 = 'b0;
    reg [15:0] div_edge_y_d2 = 'b0;
    reg [15:0] div_edge_z_d2 = 'b0;
    // last
    always @(posedge clk or posedge rst) begin
        if (rst) begin
            ray_slope_x_d2 <= 'b0;
    ...
    always @(posedge clk or posedge rst) begin
        if (rst) begin
            ...
            div_edge_x_d2 <= (ray_slope_x_d1 != 0) ? ((edge_x_d1 * recip_x_d1) >> 9) : 16'hffff;
            div_edge_y_d2 <= (ray_slope_y_d1 != 0) ? ((edge_y_d1 * recip_y_d1) >> 9) : 16'hffff;
            div_edge_z_d2 <= (ray_slope_z_d1 != 0) ? ((edge_z_d1 * recip_z_d1) >> 9) : 16'hffff;
        end
    end
*/
    ...
    always @(posedge clk or posedge rst) begin
        if (rst) begin
            ...
            div_edge_x_d2 <= (ray_slope_x_d1 != 0) ? (edge_x_d1 << 6) / (ray_slope_x_abs >> 3) : 'hffff;
            div_edge_y_d2 <= (ray_slope_y_d1 != 0) ? (edge_y_d1 << 6) / (ray_slope_y_abs >> 3) : 'hffff;
            div_edge_z_d2 <= (ray_slope_z_d1 != 0) ? (edge_z_d1 << 6) / (ray_slope_z_abs >> 3) : 'hffff;
        end
    end
